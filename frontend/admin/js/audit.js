...
async function loadAudit(page) {
  ...
  try {
    ...
    if (!logs.length) {
      t.innerHTML = '<tr><td colspan="6"><div class="alert alert-info">No audit logs found.</div></td></tr>'
      document.getElementById('auditPagination').innerHTML = ''
      return
    }
    t.innerHTML = (logs||[]).map(l => `<tr><td>${new Date(l.created_at).toLocaleString()}</td><td>${l.action}</td><td>${l.entity_type||'-'}</td><td>${l.entity_id?.slice(0,8)||'-'}</td><td>${l.details?.slice(0,100)||'-'}</td><td style="font-size:0.85rem;color:var(--text-secondary);">${l.user_name||'-'}</td></tr>`).join('')
