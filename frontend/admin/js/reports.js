async function loadReports() {
  ...
  try {
    ...
    <div class="card" style="margin-top:1.5rem;"><h3>Revenue by Event</h3><table class="table"><tr><th>Event</th><th>Revenue</th><th>Bookings</th></tr>${((r.revenue_by_event||[]).map(e => `<tr>      <td>${(e.title||'').replace(/[<>]/g,'')||'Unknown'}</td><td>₹${Number(e.revenue||0).toLocaleString()}</td><td>${e.count||0}</td></tr>`).join('')||'<tr><td colspan="3">No data</td></tr>')}</table></div>`
